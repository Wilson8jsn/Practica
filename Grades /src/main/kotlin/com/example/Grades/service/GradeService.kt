...
class GradeService {
    ...
    fun save(grade: Grade): Grade {
        try {
            // Validación de gradeName
            grade.gradeName?.takeIf { it.trim().isNotEmpty() }
                ?: throw Exception("Grade subject is null or empty")

            // Validación de gradeValue
            val numericGrade = grade.gradeValue
            numericGrade?.takeIf { it in 0.0..10.0 }
                ?: throw Exception("Grade must be between 0.0 and 10.0")

            // Guardar el objeto Grade en la base de datos
